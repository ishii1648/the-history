...
// ---- TASK-54: 密集地域のラベル背景パネル・衝突間引き強化 ----

Deno.test("LABEL_BACKGROUND_COLOR は半透明の羊皮紙系（不透明度 < 255）", () => {
  const [r, g, b, a] = LABEL_BACKGROUND_COLOR;
  // 羊皮紙の下地（basemap の地色）と調和する暖色系で、かつ地物の塗りが
  // 透けて見える程度の半透明（背景に完全に同化しない・地物色も殺さない）
  assert(
    r >= 200 && g >= 200 && b >= 150,
    `bg=${LABEL_BACKGROUND_COLOR} は明るい暖色系のはず`,
  );
  assert(a > 100 && a < 255, `bg alpha=${a} は半透明のはず`);
});

Deno.test("LABEL_BACKGROUND_PADDING は正の [x, y] ピクセル padding", () => {
  const [px, py] = LABEL_BACKGROUND_PADDING;
  assert(px > 0, `padding_x=${px} は正のはず`);
  assert(py > 0, `padding_y=${py} は正のはず`);
});

Deno.test("COLLISION_SIZE_SCALE は従来値 2 より大きい（密集地帯の間引き強化）", () => {
